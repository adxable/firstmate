...
| `fm-install.sh`          | Stand firstmate up on a machine with one command: clone or reuse the checkout, install approved missing tools through `fm-bootstrap.sh`, check the captain style wiring, and report what is left for a human |
...
| `fm-silence-sentry.sh`   | Detached per-close sentry that reports, and never restarts, a home that stopped watching while no turn ever finished the wake (docs/watcher-continuity.md) |
...
| `fm-captain-style.sh`    | Print the import line and memory file that wire the captain style contract into user-level Claude memory, and read-only `--check` that wiring |
