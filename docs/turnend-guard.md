...
Scope is resolved before the hook payload is read, and that ordering is a requirement, not an implementation detail.
The scope test is the only step that needs no payload, so an out-of-scope checkout exits 0 without ever reading stdin.
A harness may hand a hook a payload pipe it never closes, and a guard that reads stdin first would then block forever inside a checkout it was always going to leave alone, wedging the whole worker session on a turn it was never going to act on.
Crewmate and scout task worktrees inherit these tracked hooks and are the common out-of-scope case, so that wedge is the normal path, not an edge case.
Any new guard on this page, and any change to an existing one, must keep every stdin read below the scope test.
`bin/fm-turnend-guard.sh`, `bin/fm-claude-stop-autoarm.sh`, and `bin/fm-turnend-guard-cursor.sh` all follow this order, as do the sibling PreToolUse guards in [`cd-guard.md`](cd-guard.md) and [`subagent-guard.md`](subagent-guard.md).
Exiting without draining the pipe is safe in the other direction too, because the writer sees a closed read end rather than a reader that never returns.
...
- Claude registers two `Stop` hooks in `.claude/settings.json`, both anchored through `CLAUDE_PROJECT_DIR`: `bin/fm-turnend-guard.sh --claude` with `timeout: 180`, and `bin/fm-claude-stop-autoarm.sh` with `asyncRewake: true` and `timeout: 28800`.
  The guard's own declaration exists because its last-resort arm confirms a watcher synchronously inside that hook; [`watcher-continuity.md`](watcher-continuity.md#last-resort-arm-at-the-stop-boundary) owns what that ceiling cuts off and what survives it.
...
A claim is open while its outcome is `arming`, its owner pid is alive, its recorded identity successfully recomputes and matches that pid, and it is not stuck, which proves the owner hung mid-arm; `fm_autoarm_arming_stuck` in `bin/fm-wake-lib.sh` owns that proof and [`watcher-continuity.md`](watcher-continuity.md#last-resort-arm-at-the-stop-boundary) owns why its two halves use different windows.
...
When none of those proofs appears, the guard first runs its last-resort arm (`bin/fm-guard-last-resort-arm.sh`) and, when that confirms a watcher, reports `SUPERVISION RESTORED BY THE TURN-END GUARD` instead of the blind-turn banner; [`watcher-continuity.md`](watcher-continuity.md#last-resort-arm-at-the-stop-boundary) owns that contract, its `FM_ARM_CONFIRM_TIMEOUT` confirmation window, and why the Stop boundary is the only place it may run.
With or without a restored watcher it then re-blocks up to `FM_CLAUDE_TURNEND_BLOCK_BUDGET` times (default 3, below Claude's 8-block override), and once that budget is exhausted a restored watcher lets the turn end supervised rather than blind.
