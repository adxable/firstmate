...
That is exactly the state in which the return asks nothing, so the operator closing the orphaned pane gets no prompt and no warning that the holder loses its position.
