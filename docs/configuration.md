...
FM_CLAUDE_GUARD_ARM_CONFIRM=10   # seconds the --claude guard's last-resort arm waits to confirm the watcher it launched; default 30 on Git Bash/MSYS, matching FM_ARM_CONFIRM_TIMEOUT
FM_CLAUDE_AUTOARM_ARMING_BUDGET   # seconds an "arming" auto-arm claim stays credited with no watcher beacon; unset derives it from FM_ARM_CONFIRM_TIMEOUT and FM_CLAUDE_AUTOARM_ATTEMPTS with a 60s floor, and any value is clamped to FM_GUARD_GRACE
