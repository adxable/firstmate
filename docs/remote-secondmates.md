...
- The required remote tool set is `git`, `jq`, `herdr`, compatible `tasks-axi`, a `treehouse` that honors a configured pool root, and at least one of `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, or `kimi`.
