...
# Every seeded reply window is relative to this run's own clock. A literal date
# would turn each seeded thread unreachable the moment that date passed, and the
# suite would then fail on the wall clock instead of on behaviour. Tests that
# need an expired or closing window still derive it from these offsets and pin
# "now" with FMX_NOW_OVERRIDE.
FIXTURE_NOW=$(date -u +%s)
FIXTURE_RECEIVED_OFFSET=$((-1 * 86400))
FIXTURE_WINDOW_OFFSET=$((30 * 86400))
FIXTURE_RESERVATION_OFFSET=$((60 * 86400))

fixture_iso() {  # <offset-seconds-from-FIXTURE_NOW>
  local epoch=$((FIXTURE_NOW + $1))
  # BSD date reads an epoch with -r; GNU date rejects that and takes -d @epoch.
...
seed_commitment() {
  ...
  jq -n --arg r "$request" --arg p "$platform" \
    --arg received "$(fixture_iso "$FIXTURE_RECEIVED_OFFSET")" \
    --arg window "$(fixture_iso "$FIXTURE_WINDOW_OFFSET")" \
    '{request_id:$r, platform:$p,
      ...
      followup_expires_at:$window,
      reservation_expires_at:$window}' > "$home/request.json"
  ...
  tasks_in "$home" public-followup add "$obligation" \
    ...
    --expires-at "$(fixture_iso "$FIXTURE_RESERVATION_OFFSET")" >/dev/null \
...
seed_repro_commitment() {   # <home> <obligation> <request> <work-home> <work-id>
  ...
  tasks_in "$home" public-followup add "$obligation" --request-context-file "$home/request.json" \
    ...
    --expires-at "$(fixture_iso "$FIXTURE_RESERVATION_OFFSET")" >/dev/null || fail "add failed"
...
test_rechain_claims_delivered_source_once() {
  ...
  FMX_NOW_OVERRIDE="$FIXTURE_NOW" run_pf "$home" rechain public-final-claim-b \
    --from public-final-claim-a --work-home main --work-id ship-claim-b \
    --expected pr-merged > "$home/rechain-b.out" 2>&1 &
  pid_b=$!
  FMX_NOW_OVERRIDE="$FIXTURE_NOW" run_pf "$home" rechain public-final-claim-c \
...
test_expiry_escalation_uses_now_override() {
  ...
  exp=$((FIXTURE_NOW + FIXTURE_WINDOW_OFFSET))
