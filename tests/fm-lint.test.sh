...
test_parse_guard_covers_the_whole_file_set() {
  ...
  # CI=true selects the full canonical set explicitly, so this stays a whole-tree
  # assertion on a local branch where the default file set is the changed one.
  out=$(CI=true "$LINT" --parse-guard 2>&1) || rc=$?
  [ "$rc" -eq 0 ] \
...
# Changed-file selection can hand the guard an empty root set, and Bash 3.2 - the
# shell the guard exists for - treats "${ROOTS[@]}" as unset under `set -u`, so an
# unguarded expansion aborts there while passing on any modern Bash. The note is
# what makes the empty case observable on every shell.
test_parse_guard_notes_zero_changed_targets() {
  local tmp fakebin diff_file out rc=0 bash32
  tmp=$(fm_test_tmproot fm-lint-guard-zero-changed)
  fakebin=$(fm_fakebin "$tmp")
  fm_lint_stub_git "$fakebin"
  diff_file="$tmp/diff.nul"
  : > "$diff_file"
  ...
  out=$(PATH="$fakebin:$PATH" GITHUB_ACTIONS='' CI='' FM_TEST_GIT_BRANCH=feature \
    FM_TEST_GIT_DIFF_FILE="$diff_file" "$LINT" --parse-guard 2>&1) || rc=$?
  [ "$rc" -eq 0 ] || fail "the parse guard failed on an empty changed set, got $rc"$'\n'"$out"
  assert_contains "$out" "no changed lint targets" \
    "the parse guard did not note the empty target set"

  if bash32=$(fm_lint_bash32); then
    rc=0
    out=$(PATH="$fakebin:$PATH" GITHUB_ACTIONS='' CI='' FM_TEST_GIT_BRANCH=feature \
      FM_TEST_GIT_DIFF_FILE="$diff_file" "$bash32" "$LINT" --parse-guard 2>&1) || rc=$?
    [ "$rc" -eq 0 ] \
      || fail "the parse guard failed on an empty changed set under $bash32, got $rc"$'\n'"$out"
    assert_not_contains "$out" "unbound variable" \
      "the parse guard expanded an empty root array under $bash32"
    pass "the parse guard notes an empty changed set instead of expanding it (confirmed on $bash32)"
    return
  fi
  pass "the parse guard notes an empty changed set instead of expanding it (no 3.2 available to confirm)"
}
