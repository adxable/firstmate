...
| `/software-factory` | Run the four-gate feature workflow (Product, Architecture, Program Design, Vertical Slices): one scout drafts each gate, you approve it in a Lavish review surface, and each approved slice ships as its own chained task and PR; also offered once at intake when a feature-sized request has open product or design questions |
