...
config/treehouse-root  optional absolute path overriding this home's own worktree-pool root; LOCAL, gitignored, and NOT inherited by secondmate homes (one shared root is what lets two homes be handed each other's worktrees); see docs/configuration.md "Worktree pool root"
...
The spawn must resolve a genuine isolated task worktree distinct from the primary checkout and not already recorded as the worktree of another task whose endpoint is proven to still exist; a failed isolation assertion stops the task.
