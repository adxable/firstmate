...
- Run `bin/fm-lint.sh` before treating a script change as done; it is the single owner of the lint definition (file set, config, pinned shellcheck version, and the structural rules shellcheck does not model, such as the Bash 3.2 command-substitution heredoc guard behind `--parse-guard`) that CI and the no-mistakes pre-push gate both invoke, and it refuses to run under any other shellcheck version.
