...
- Helper scripts in `bin/` are plain bash.
  ...
  It also applies one structural rule shellcheck does not model: a heredoc nested inside `$( ... )` must not leave Bash 3.2's quote or paren state unbalanced, because stock macOS `/bin/bash` then parses the rest of the file as that leftover state.
  Run just that rule with `bin/fm-lint.sh --parse-guard`; the `macos-stock-bash` CI job remains the authoritative cross-version proof.
