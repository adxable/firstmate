...
When you edit a PR description by hand or with an agent, keep the section no-mistakes appended intact instead of replacing the whole body, because the check reads the body rather than the commits, so a dropped signature or attestation has to be restored there - from the description's edit history, from the log of the run that accepted it, or by another pipeline run.
...
- Helper scripts in `bin/` are plain bash.
  ...
  `bin/fm-lint.sh` must pass: it is the single owner of the lint definition (the shellcheck file set, config, pinned shellcheck version, pinned actionlint workflow lint, and the structural rules shellcheck does not model), and both CI and the no-mistakes pre-push gate run its no-argument full-analysis path.
  ...
  One of those structural rules is that a heredoc nested inside `$( ... )` must not leave Bash 3.2's quote or paren state unbalanced, because stock macOS `/bin/bash` then parses the rest of the file as that leftover state; the `macos-stock-bash` CI job remains the authoritative cross-version proof.
