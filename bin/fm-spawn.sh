...
#   A resolved worktree already recorded as the worktree= of another task whose
#   endpoint is proven to still exist is refused the same way; what that refusal
#   does with its own endpoint is backend-specific and owned by
#   discard_refused_endpoint's header.
...
# An adopted endpoint (SPAWN_ADOPTED_ENDPOINT=1, a --relaunch that reuses its
# recorded endpoint rather than rebinding a proven-gone one) is left
