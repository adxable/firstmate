...
When a decision you escalated is answered or a blocker clears and your domain resumes, append \`resolved [key=<slug>]: {how it was decided or unblocked}\` with the same key you opened it with, or a bare \`resolved: {how it was decided or unblocked}\` if you opened it without one, so it is durably closed instead of resurfacing behind later unrelated events.
...
6. If a decision belongs to a human (product choices, destructive actions),
   ...
   To keep more than one open at a time, put the key BEFORE the colon on both lines - \`needs-decision [key=<slug>]: {summary of options}\` and later \`resolved [key=<slug>]: {how it was decided or unblocked}\` - because a key written after the colon is not read and every keyed decision collapses into one.
